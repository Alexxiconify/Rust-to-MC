...
public final class ImmediatelyFastCompat {
    ...
    // ── Query API for other mixins: True when IF it is handling HUD draw call batching — we should skip our own HUD optimizations.
    ...
    // IF makes each draw call cheaper through batching, so we can afford more visible entities. <li>IF active + HUD batching: 1.3x (30% more entities visible)</li>
    //<li>IF active without HUD batching: 1.15x</li>  <li>IF not present: 1.0x (no change)</li>
